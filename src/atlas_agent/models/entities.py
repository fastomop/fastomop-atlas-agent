...
class ClinicalEntity(BaseModel):
    ...
    ] = Field(description="Type of clinical entity")
    ...
    domain: Literal[
        ...
        "Race",  # Race
        "Ethnicity",  # Ethnicity
    ] = Field(description="OMOP domain for searching")

    is_required: bool = Field(default=True, description="Whether this entity MUST be in the concept set")

    requires_descendants: bool = Field(
        default=True, description="Whether to include descendant concepts (ATLAS includeDescendants)"
    )

    is_exclusion: bool = Field(default=False, description="Whether this should be excluded from the concept set")
    ...
    rationale: str = Field(description="Clinical reasoning for why this entity was extracted")
...
class ParsedClinicalDescription(BaseModel):
    ...
    entities: list[ClinicalEntity] = Field(description="Extracted clinical entities")

    interpretation: str = Field(description="Overall clinical interpretation of the description")

    concept_set_strategy: str = Field(description="High-level strategy for building the concept set")
