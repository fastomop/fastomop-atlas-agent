...
schema = {
    "type": "object",
    "properties": {"name": {"type": "string"}, "genre": {"type": "string"}, "year": {"type": "integer"}},
    "required": ["name", "genre", "year"],
}

response = client.chat(
    model="gpt-oss:20b",
    messages=[{"role": "user", "content": f"Generate a movie JSON matching this schema: {json.dumps(schema)}"}],
    format=schema,
)
print(f"Response: {response['message']['content']}")
try:
